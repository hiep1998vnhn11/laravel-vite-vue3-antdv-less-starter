...
const getters = {
  getErrorLogInfoList: (state: ErrorLogState): ErrorLogInfo[] =>
    state.errorLogInfoList || [],
  getErrorLogListCount: (state: ErrorLogState): number =>
    state.errorLogListCount,
}
const actions = {
  /**
   ...
   * @returns
   */
  addAjaxErrorInfo({ commit }: { commit: Commit }, error: any) {
