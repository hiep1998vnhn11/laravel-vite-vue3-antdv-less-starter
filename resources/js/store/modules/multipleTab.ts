...
function goToPage(state: MultipleTabState, router: Router) {
    const go = useGo(router)
...
const getters = {
    ...
    getCachedTabList: (state: MultipleTabState): string[] =>
        Array.from(state.cacheTabList),
    getLastDragEndIndex: (state: MultipleTabState): number =>
        state.lastDragEndIndex,
}
...
const actions = {
     ...
     */
    async refreshPage(
        { commit, getters }: { getters: any; commit: Commit },
        router: Router
    ) {
        ...
        const findTab = getters.getCachedTabList.find(
    ...
    async addTab(
        { state, commit }: { commit: Commit; state: MultipleTabState },
    ...
    // Sort the tabs
    async sortTabs(
        { state, commit }: { state: MultipleTabState; commit: Commit },
    ...
    async closeLeftTabs(
        { commit, state }: { commit: Commit; state: MultipleTabState },
        { tab, router }: TabAction
    ) {
    ...
    async closeRightTabs(
        ...
        if (index >= 0 && index < state.tabList.length - 1) {
            const rightTabs = state.tabList.slice(
                index + 1,
    ...
    async closeOtherTabs(
        ...
        const closePathList = state.tabList.map((item: any) => item.fullPath)

        const pathList: string[] = []

        for (const path of closePathList) {
            if (path !== tab.fullPath) {
                const closeItem = state.tabList.find(
...
const mutations = {
    ...
    DELETE_TAB: function (state: MultipleTabState, findTab: any) {
