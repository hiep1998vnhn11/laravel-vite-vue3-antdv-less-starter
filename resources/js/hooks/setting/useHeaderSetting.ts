...
export function useHeaderSetting() {
  ...
  const getShowDoc = computed(
    () => store.getters['app/getHeaderSetting'].showDoc
  )

  const getHeaderTheme = computed(
    () => store.getters['app/getHeaderSetting'].theme
  )

  const getShowHeader = computed(
    () => store.getters['app/getHeaderSetting'].show
  )

  const getFixed = computed(() => store.getters['app/getHeaderSetting'].fixed)

  const getHeaderBgColor = computed(
    () => store.getters['app/getHeaderSetting'].bgColor
  )

  const getShowSearch = computed(
    () => store.getters['app/getHeaderSetting'].showSearch
  )

  const getUseLockPage = computed(
    () => store.getters['app/getHeaderSetting'].useLockPage
  )

  const getShowFullScreen = computed(
    () => store.getters['app/getHeaderSetting'].showFullScreen
  )

  const getShowNotice = computed(
    () => store.getters['app/getHeaderSetting'].showNotice
  )
